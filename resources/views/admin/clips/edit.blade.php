...
@section('content')
    ...
    {!! Form::model($clip, ['method' => 'PUT', 'route' => ['admin.clips.update', $clip->id]]) !!}
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <table class="table table-bordered table-striped">
                ...
                <tbody id="videos">
                    @forelse(old('videos', []) as $index => $data)
                        @include('admin.clips.videos_row', [
                            'index' => $index
                        ])
                    @empty
                        @foreach($clip->videos as $item)
                            @include('admin.clips.videos_row', [
                                'index' => 'id-' . $item->id,
                                'field' => $item
                            ])
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <table class="table table-bordered table-striped">
                ...
                <tbody id="images">
                    @forelse(old('images', []) as $index => $data)
                        @include('admin.clips.images_row', [
                            'index' => $index
                        ])
                    @empty
                        @foreach($clip->images as $item)
                            @include('admin.clips.images_row', [
                                'index' => 'id-' . $item->id,
                                'field' => $item
                            ])
